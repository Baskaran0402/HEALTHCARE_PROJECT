def explain_risk(disease, patient):
    ...
    if disease == "Diabetes":
        if safe_patient.get("hba1c", 0) >= 6.5:
            explanations.append(
                "Elevated HbA1c suggests poor long-term glucose control."
            )
        ...
        if safe_patient.get("bmi", 0) >= 30:
            explanations.append(
                "Increased BMI is a known risk factor for insulin resistance."
            )
        ...
        if safe_patient.get("chest_pain", 0) == 1:
            explanations.append(
                "Presence of chest pain raises concern for cardiac origin."
            )
        ...
        explanations.append(
            "Abnormal liver-related indicators were detected in the model input."
        )
    ...
    if not explanations:
        explanations.append(
            "Risk inferred based on combined clinical feature patterns."
        )
