def adapt_stroke_features(patient):
    ...
    return {
        ...
        "hypertension": int(patient.get("hypertension")) if patient.get("hypertension") is not None else 0,
        "heart_disease": int(patient.get("heart_disease")) if patient.get("heart_disease") is not None else 0,
        "work_type": patient.get("work_type") if patient.get("work_type") is not None else 0,
        "avg_glucose_level": float(patient.get("blood_glucose")) if patient.get("blood_glucose") is not None else 100.0,
        "bmi": float(patient.get("bmi")) if patient.get("bmi") is not None else 24.0,
    }
