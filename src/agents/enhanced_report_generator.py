...
def generate_comprehensive_doctor_report(ml_report: dict, patient_name: str = "Patient") -> str:
    ...
    report_sections.append("")
    report_sections.append(f"**Overall Risk Stratification:** {overall_risk.get('level', 'Unknown')} ({overall_risk.get('score', 0):.1f}%)")
...
def _generate_executive_summary(overall_risk: dict, individual_risks: list) -> str:
    ...
    if level == 'Critical' or critical_count > 0:
        return f"This health screening has identified **{level.lower()} risk** areas that require prompt medical attention. {critical_count + high_count} condition(s) show elevated risk levels. We strongly recommend scheduling a consultation with your healthcare provider for comprehensive evaluation and personalized care planning."
    elif level == 'High' or high_count > 0:
        return f"This screening shows **{level.lower()} risk** in certain health areas. {high_count} condition(s) have elevated risk indicators. We recommend consulting with your healthcare provider to discuss these findings and develop an appropriate monitoring or intervention plan."
    elif level == 'Moderate':
        return f"This screening indicates **moderate risk** in some health areas. While not immediately concerning, these findings suggest opportunities for preventive care and lifestyle optimization. Regular monitoring and healthy habits are recommended."
    else:
        return f"This screening shows **low risk** across assessed health areas. Continue maintaining your current healthy practices and schedule regular check-ups as recommended by your healthcare provider."
...
def _get_disclaimer() -> str:
    ...
    return """**This report is generated by an AI-powered health screening tool and is intended for informational and educational purposes only.**
...
**Accuracy Note:** AI models are trained on large datasets but may not account for all individual variations. Clinical judgment by a qualified healthcare professional is essential for accurate diagnosis and treatment planning."""


def _get_professional_disclaimer() -> str:
    """Get professional disclaimer for doctors"""
    return """**Clinical Decision Support Tool - Professional Use Only**

This report is generated by machine learning models trained on clinical datasets. The risk scores and assessments are intended to support, not replace, clinical judgment.
...
**Liability:** This tool provides decision support only. Final diagnostic and treatment decisions remain the responsibility of the attending physician."""
