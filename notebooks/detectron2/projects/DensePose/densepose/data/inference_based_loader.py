...
class InferenceBasedLoader:
    ...
    def _produce_data(
        ...
        for batch in batched_images_and_categories:
            ...
            model_output_filtered = model_output if self.data_filter is None else self.data_filter(model_output)
            data = model_output_filtered if self.data_sampler is None else self.data_sampler(model_output_filtered)
            for data_i in data:
