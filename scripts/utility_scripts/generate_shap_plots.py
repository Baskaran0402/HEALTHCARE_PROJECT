...
def main():
    ...
    try:
        generate_shap_plots("Heart Disease", "models/heart_model.pkl", heart_sample, heart_features)
    ...
    try:
        generate_shap_plots("Diabetes", "models/diabetes_model.pkl", diabetes_sample, diabetes_features)
    except Exception as e:
        print(f"❌ Error generating Diabetes SHAP plots: {e}")

    # Stroke Model
    stroke_features = ["age", "hypertension", "heart_disease", "avg_glucose_level", "bmi", "gender", "smoking_status"]
    stroke_sample = np.random.rand(100, len(stroke_features))  # Replace with real data

    try:
        generate_shap_plots("Stroke", "models/stroke_model.pkl", stroke_sample, stroke_features)
    except Exception as e:
