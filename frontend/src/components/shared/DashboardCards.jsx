...
export function ActionCard({ title, desc, badge, badgeColor, path, accent, icon }) {
  ...
  return (
    <div 
      ...
      className="bg-white rounded-2xl border border-[#e8ede9] p-6 cursor-pointer transition-all duration-200 relative overflow-hidden group"
      onMouseEnter={e => {
        e.currentTarget.style.transform = 'translateY(-2px)'
        e.currentTarget.style.boxShadow = `0 12px 32px ${accent}18`
    ...
    >
      ...
      <p className="text-xs text-[#0a0a0f]/45 m-0 mb-4 leading-relaxed font-inter">{desc}</p>
      <span 
  ...
  return (
    ...
    >
      ...
      <div>
        ...
        <p className="text-[0.72rem] text-[#0a0a0f]/40 m-0 mt-0.5">{desc}</p>
      </div>
    </div>
  )
}
